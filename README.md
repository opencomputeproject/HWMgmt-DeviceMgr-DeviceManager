...
```shell
#> sudo systemctl restart docker      #If you encountered that fails to download images, you need to use this command to fix it.
```
...
1. Why the build dm/mockup could not work?

   Solution: That means you should check the building the "k8s" with the CALICO_IPAM option. It could not conflict with "docker0" and host subnets.
