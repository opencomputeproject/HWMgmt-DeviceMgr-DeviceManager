...
int embedded = FALSE;
//ec-dashboard: Disable table header
...
void document_header(int use_stylesheet) {
	...
	printf("    function openwin(hostname, status, address) {\n");
	printf("        url='/nagios/ec_more_info.php?host_name=' + hostname + '&status=' + status + '&address=' + address\n");
	printf("        var PosX = (screen.width-500)/2;\n");
	printf("        var PosY = (screen.hight-300)/2;\n");
	printf("        window.open (url, 'newwindow', 'height=300, width=500,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no, top=' + PosY + ', left=' + PosX);\n");
	printf("    }\n");
        printf("    function alertmsg(hostname){\n");
	printf("    if (hostname=='client01'||\n");
	printf("        hostname=='client02'||\n");
	printf("        hostname=='client03'||\n");
	printf("        hostname=='client04'||\n");
	printf("        hostname=='client05'||\n");
	printf("        hostname=='client06'||\n");
	printf("        hostname=='client07'||\n");
	printf("        hostname=='client08'||\n");
	printf("        hostname=='client09'||\n");
	printf("        hostname=='client10'){\n");
	printf("        alert('Power Supply Alert: Unit at Slot #0 is malfunctioning');\n");
	printf("    } else if ( hostname=='client11'||\n");
	printf("                hostname=='client12'||\n");
        printf("                hostname=='client13'||\n");
	printf("                hostname=='client14'||\n");
	printf("                hostname=='client15'||\n");
	printf("                hostname=='client16'||\n");
	printf("                hostname=='client17'||\n");
	printf("                hostname=='client18'||\n");
	printf("                hostname=='client19'||\n");
	printf("                hostname=='client20'){\n");
	printf("       alert('Thermal Alert: Internal temperature is above 60C');\n");
	printf("    } else { alert('Fan Alert: Unit 4B is operating at 1200 RPM'); }\n");
	printf("    }\n");
	printf("</script>\n");
...
void show_service_detail(void) {
	...
	// ec-dashboard: disable orignal page footer
	//int user_has_seen_something = FALSE;
	int user_has_seen_something = TRUE;
	...
	//ec-dashboard: Modify host statsu page title
	//printf("<div align='center' class='statusTitle'>Service Status Details For ");
	//printf("<div align='center' class='statusTitle'>Host Status For ");
        printf("<div align='center' class='statusTitle'>Seattle Data Center ");
	/*  
	if(display_type == DISPLAY_HOSTS) {
		if(show_all_hosts == TRUE)
			printf("All Hosts");
	...
	*/
	...
	// ec-dashboard: Layout table header of host status web page
	printf("<th class='status'>Status</th>\n");
	printf("<th class='status'>Device</th>\n");
	printf("<th class='status'>Device Type</th>\n");
	printf("<th class='status'>Details</th>\n");

	// ec-dashboard: comment the orignal table header
/*
...
*/
    ...
    // ec-dashboard: Layout table rows of host status web page
    // go through each nagios hosts
    temp_host = host_list;
    for(temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
        if(is_authorized_for_host(temp_host, &current_authdata) == FALSE)
	    continue;
        ...
        // Field 1: Display host link status 
        // Get the host status information
        hoststatus *temp_hoststatus = find_hoststatus(temp_host->name);
        printf("<td class='status%s'>", host_status_bg_class);

        if(temp_hoststatus->status == SD_HOST_UP) {
            printf("<img src='/nagios/images/enabled.gif' alt='Link Up'>");
        } else {
            //printf("<a href='#' onclick='openwin(\"%s\",\"down\")'><img src='/nagios/images/disabled.gif' alt='Link Down'></a>",temp_host->name);
            printf("<a href='#' onclick='alertmsg(\"%s\")'><img src='/nagios/images/disabled.gif' alt='Link Down'></a>",temp_host->name);
        }
        printf("</td>");

        // Field 2: Display host name and IP address  
        printf("<td>%s<br>[%s]</td>", temp_host->name, temp_host->address);

        // Field 3: Display host device type
        printf("<td>");
        if(!strcmp(temp_host->name, "localhost")) {
            printf("<img src='/nagios/images/ubuntu-48.png' style='width:40px;height:40px;' alt='Linux Host'>");
	} else {
           printf("<img src='/nagios/images/router.png' style='width:40px;height:40px;' alt='Network Device'>");
        }
        printf("</td>");
	    ...
	    // Only process the same host name
	    if(strcmp(temp_status->host_name, temp_host->name)) {
	        continue;
	    }

    	    // debug info.
	    // printf("<br>Host Name: %s", temp_status->host_name);
	    
	    
	    // temp solution, select the service name which we wanted
	    if(strcmp(temp_status->description, "Serial Number")) {
                continue;
            }

	    // debug info
	    //printf("<br>%s: %s", temp_status->description,
	    //		    temp_status->current_attempt == 0 ? "OK" :
	    //		    temp_status->current_attempt == 1 ? "Warning" :
	    //		    temp_status->current_attempt == 2 ? "Alert" :
	    //		    temp_status->current_attempt == 3 ? "Critical" : "Unknown");
            printf("%s: %s<br>", temp_status->description, (temp_status->plugin_output == NULL) ? "" : html_encode(temp_status->plugin_output, TRUE));
        } // End of service
        printf("</td>");

	// Field 5: More Info. (OCP Baseline Data)
	printf("<td>");
	//printf("<a href='/nagios/ec_more_info.php?host_name=%s' target='_blank'><img src='/nagios/images/more.png' style='width:30px;height:30px;' alt='OCP Basedline Data'></a>", temp_host->name);
	printf("<a href='#' onclick='openwin(\"%s\",\"up\",\"%s\")'><img src='/nagios/images/more.png' style='width:30px;height:30px;' alt='OCP Basedline Data'></a>", temp_host->name, temp_host->address);
	printf("</td>");
	...
	/* check all services... */
   // ec-dashboard: comment the orignal table rows
   while(0) {
// while(1) {
